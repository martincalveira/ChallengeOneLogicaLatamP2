...
var intentos = 0;
var palabraAdivinar = bDPalabras[Math.floor(Math.random() * bDPalabras.length)].toUpperCase();
...
function construirTr(dato){
       var tr = document.createElement("tr");         
       tr.appendChild(construirTd(dato));
       return tr; 
};

function construirTrGuiones(){
    var trGuiones = document.getElementById("guiones");
    //var cadaGuion = trGuiones.getElementsByTagName("td");
    for (let i = 0; i < letrasAdivinadas.length; i++){
        trGuiones.appendChild(construirTd(GUION));
    }
};

function construirTrLetrasCorrectas(){
